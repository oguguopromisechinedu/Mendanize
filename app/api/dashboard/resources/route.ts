...
import { upsertFreeResource } from "@/services/platform"
...
const schema = z.object({
  ...
  type: z.enum(["PDF", "TEMPLATE", "CHECKLIST", "CHEATSHEET", "EBOOK"]),
...
export async function POST(req: Request) {
  try {
    ...
    if (!parsed.success) {
      return fail("VALIDATION_ERROR", "Invalid resource payload.", 400)
    }
    const row = await upsertFreeResource(parsed.data)
    return ok(row)
  } catch (error) {
